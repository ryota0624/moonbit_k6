# test_on_k6 - k6 MoonBit Library Test Suite

このディレクトリは、MoonBitで実装したk6ライブラリの動作を**実際のk6ランタイム上で検証する**ためのテストスイートです。

## 目的

- MoonBitで書いたシナリオが意図通りに動作するかを確認
- k6のレポート出力を使って期待値との整合性を検証
- `moon test`ではなく、実際のk6環境で実行

## 特徴

### ✅ 期待値駆動のテスト設計

各テストには「期待される動作」がコメントで明記されています：

```moonbit
// 期待される動作:
// - VU: 1
// - Iterations: 3
// - http_reqs: 6 (2 requests per iteration)
// - checks: 9 (3 checks per iteration)
```

### ✅ k6レポートで検証

テスト実行後のk6レポートで、期待値と実際の値を比較します：

```
checks.........................: 100.00% ✓ 9 ✗ 0
http_reqs......................: 6       X/s
test_counter...................: 3       X/s
```

## クイックスタート

### 🚀 自動検証付きCI実行（推奨・ローカル環境）

```bash
npm run ci:verify:local
```

このコマンドは以下を自動実行：
1. `npm run build` - テストコードをビルド（MoonBit → JS）
2. `npm run build:verify` - 検証スクリプトをビルド（MoonBit → JS）
3. `npm run docker:up` - httpbinコンテナ起動
4. `npm run test:json` - k6実行 + summary.json生成
5. `npm run docker:down` - httpbinコンテナ停止
6. `npm run verify` - **期待値との自動検証**

**期待値と一致しない場合、exit code 1で終了します。**

### 🐳 Docker環境について

test_on_k6は**ローカルのhttpbinコンテナ**に対してテストを実行します。外部ネットワークへの依存がないため、安定したテスト実行が可能です。

**環境変数 `HTTPBIN_URL`:**
- 未設定: `http://localhost:8080` （デフォルト）
- 設定例: `HTTPBIN_URL=https://httpbin.org npm run ci:verify`

### ワンコマンドで全テスト実行（自動検証なし）

```bash
npm run ci
```

このコマンドは以下を実行します：
1. `npm install` - 依存関係のインストール
2. `npm run build` - MoonBit → JS変換（vite）
3. `npm run test` - k6でテスト実行（目視確認）

### 個別ステップでの実行

```bash
# 1. 依存関係のインストール
npm install

# 2. httpbinコンテナを起動
npm run docker:up

# 3. テストコードをビルド
npm run build

# 4. 検証スクリプトをビルド
npm run build:verify

# 5. k6でテスト実行 + JSON出力
npm run test:json

# 6. 自動検証
npm run verify
# → ✅ All expectations passed! または ❌ エラー表示

# 7. httpbinコンテナを停止
npm run docker:down
```

### Docker Compose コマンド

```bash
# httpbinコンテナを起動
npm run docker:up

# httpbinコンテナのログを表示
npm run docker:logs

# httpbinコンテナを停止
npm run docker:down
```

## 🤖 自動検証機能

test_on_k6は、k6の実行結果を自動的に検証する機能を備えています。

### 仕組み

1. **期待値の定義** (`expectations.json`)
   ```json
   {
     "testName": "k6 MoonBit Library Test",
     "expectations": [
       {
         "metric": "http_reqs",
         "path": "metrics.http_reqs.count",
         "expected": 6,
         "type_": "exact"
       }
     ]
   }
   ```

2. **k6実行とJSON出力**
   ```bash
   k6 run --summary-export=summary.json dist/test.js
   ```

3. **MoonBit検証スクリプト実行**
   ```bash
   node verify/_build/js/release/build/k6-verify.js
   ```

   - `summary.json`を読み込み
   - `expectations.json`と比較
   - 一致: exit 0
   - 不一致: exit 1 + エラー詳細

### 検証タイプ

- `exact`: 完全一致
- `min`: 最小値以上
- `max`: 最大値以下

### 出力例

**成功時:**
```
✓ iterations: 3 (expected: 3)
✓ http_reqs: 6 (expected: 6)
✓ checks_value: 1 (expected: 1)
✅ All expectations passed!
```

**失敗時:**
```
✗ http_reqs: 5 (expected: 6)
❌ 1 expectation(s) failed:
  - http_reqs:
      Expected: 6
      Actual:   5
```

## テスト内容

`src/test_all.mbt` に全てのテストが含まれています：

### 1. Core API Tests
- `group()` - グループ化機能
- `check()` - レスポンス検証
- `sleep()` - 待機機能

### 2. HTTP Tests
- `quick_get()` - HTTPリクエスト
- `check_all()` - 複数のcheck
- レスポンス処理

### 3. Metrics Tests
- `Counter` - カウンタメトリクス
- カスタムメトリクスの記録

### 4. Lifecycle Tests
- `on_first_iteration()` - 初回イテレーション
- `every_n_iterations()` - N回ごとの実行

## k6レポートの読み方

### メトリクスの確認項目

| メトリクス | 説明 | 期待値の例 |
|-----------|------|-----------|
| `iterations` | 実行されたイテレーション数 | `3` |
| `http_reqs` | HTTPリクエスト総数 | `6` |
| `checks` | check成功率 | `100% ✓ 9 ✗ 0` |
| `test_counter` | カスタムカウンタ（例） | `3` |
| `http_req_failed` | HTTPリクエスト失敗率 | `0%` |

### 成功判定

テストが意図通りに動作している場合：
- ✅ `checks`: 100% (全て成功)
- ✅ `http_req_failed`: 0%
- ✅ 各メトリクスが期待値と一致

## ディレクトリ構成

```
test_on_k6/
├── package.json           # npm設定、スクリプト定義
├── vite.config.js         # viteビルド設定
├── moon.mod.json          # MoonBitモジュール設定
├── index.js               # エントリポイント
├── src/
│   ├── moon.pkg           # パッケージ設定
│   └── test_all.mbt       # 全テストスイート
├── verify/                # ← NEW: 検証スクリプト（MoonBit実装）
│   ├── moon.mod.json
│   ├── src/
│   │   ├── moon.pkg
│   │   └── main.mbt       # 検証ロジック
│   └── _build/            # ビルド成果物
├── expectations.json      # ← NEW: 期待値定義
├── summary.json           # k6が生成（自動生成）
├── dist/                  # ビルド成果物（自動生成）
│   └── test.js
└── README.md              # このファイル
```

## 開発フロー

### 新しいテストの追加

1. `src/test_all.mbt` を編集
2. 期待値をコメントで明記
3. ビルド・実行
4. k6レポートで検証

```moonbit
@k6.group("New Test", fn() {
  // 期待: http_reqs += 1
  let res = @k6.quick_get("https://httpbin.org/get")

  // 期待: check成功
  let _check = @k6.check_status_ok(res)
})
```

### トラブルシューティング

**ビルドエラーが出る場合:**
```bash
# MoonBit依存関係の更新
moon install
moon build --target js
```

**k6が見つからない場合:**
```bash
# k6のインストール
# macOS:
brew install k6

# その他:
https://k6.io/docs/getting-started/installation/
```

## CI/CD統合

GitHub Actionsではserviceコンテナとしてhttpbinが起動します：

```yaml
# .github/workflows/test.yml
jobs:
  test:
    runs-on: ubuntu-latest
    services:
      httpbin:
        image: kennethreitz/httpbin:latest
        ports:
          - 8080:80
    steps:
      - name: Run k6 tests
        working-directory: test_on_k6
        env:
          HTTPBIN_URL: http://localhost:8080
        run: npm run ci:verify
```

ローカル環境では`docker-compose.yml`を使用してhttpbinを起動します。

## example-vite との違い

| | test_on_k6 | example-vite |
|---|-----------|--------------|
| **目的** | ライブラリの機能検証 | ユーザー向けサンプル |
| **対象** | 開発者 | ユーザー |
| **内容** | 機能網羅的なテスト | 実用的なシナリオ例 |
| **実行** | CI/CD自動テスト | マニュアル実行、学習用 |
| **VU数** | 1 | 2〜10 |
| **期間** | 短時間（数秒） | 長時間（20秒〜） |

## 参考資料

- [k6 Documentation](https://k6.io/docs/)
- [k6 Metrics](https://grafana.com/docs/k6/latest/using-k6/metrics/)
- [MoonBit](https://www.moonbitlang.com/)
- プロジェクトルートのAPI実装（`../k6.mbt`, `../http.mbt` など）
